...
import type { ComparisonResult, OrderItem, InvoiceItem, Discrepancy } from '../../types';
...
const severityConfig: Record<string, { label: string; classes: string }> = {
  critical: { label: 'Критично', classes: 'bg-red-50 text-red-700 border border-red-200' },
  warning: { label: 'Внимание', classes: 'bg-amber-50 text-amber-700 border border-amber-200' },
  info: { label: 'Инфо', classes: 'bg-sky-50 text-sky-700 border border-sky-200' },
};
...
export default function MaterialRow({ result, orderItem, invoiceItem, index }: MaterialRowProps) {
  ...
  const hasDetails = result.reasoning || discrepancies.length > 0 || result.quantity_status;
